...
releases_uri=https://api.github.com/repos/Astrogamer54/now-playing-server/releases

# If a tag was passed as an argument, use that, otherwise fetch the latest release
if [ $# -gt 0 ]; then
	tag=$1
else
	# Fetch the latest beta release from the releases page
	tag=$(curl -LsH 'Accept: application/json' $releases_uri)
